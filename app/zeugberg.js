...
Zeugberg.prototype = {
  ...
  loadReporters: function( reporters ) {
  ...
  runTests: function( suite, callback ) {
    ...
    function next( error, failedSuite ) {
      ...
      // Bail out of current suite on first failure
      if( suite.bail && error != null ) {
        log( 'BAIL', self.currentTest.title )
        self.emit( 'bail', self.currentTest, error, failedSuite )
      ...
      self.emit( 'test', test )
      // TODO:
      // - Consider running hooks for pending tests as well (?)
      if( test.isPending ) {
      ...
      self.runHookDown( 'beforeEach', function( error, failedSuite ) {
        ...
        test.run( function( error ) {
          ...
          self.runHookUp( 'afterEach', function( hookError, failedSuite ) {
            self.emit( 'test:end', test, error || hookError )
            next( error || hookError, failedSuite || self.currentSuite )
          })
        })
      })
    }
