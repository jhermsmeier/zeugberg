...
Chrome.prototype = {
    ...
    var inspector = this.inspector = new Inspector(
      ...
      function() {
        ...
        async.series([
          ...
          function( done ) { inspector.Network.setCacheDisabled( true, done ) },
          function( done ) { inspector.Network.clearBrowserCookies( done ) },
    ...
    this.once( 'inspector:enabled', function() {
      ...
      this.inspector.Network.on( 'responseReceived', function( event ) {
        if( event.response.status )
          log( 'RESPONSE', event.response.status, event.response.statusText, event.response.url )
        if( self.loading && event.type === 'Document' ) {
          self.emit( 'response', event.response )
        }
      })
      
      this.inspector.Network.on( 'loadingFailed', function( event ) {
        log( 'LOADING:FAILED', event )
      })
      
      this.inspector.Page
        .on( 'frameNavigated', function( event ) {
          log( 'FRAME:NAVIGATED', event )
          self.loading = false
          self.emit( 'frameNavigated' )
        })
        .on( 'frameStartedLoading', function( event ) {
          log( 'FRAME:STARTED:LOADING', event )
          self.loading = true
          self.emit( 'frameLoading' )
        })
        .on( 'frameStoppedLoading', function( event ) {
          log( 'FRAME:STOPPED:LOADING', event )
          self.loading = false
  ...
  onNewWinPolicy: function( frame, url, policy ) {
    ...
    // - [ ] Handle JS dialogs (confirm, alert, etc.) (see Inspector API)
  },
